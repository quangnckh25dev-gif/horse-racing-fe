...
const RACE_STATUS_CONFIG = {
  Scheduled:        { label: "Đã lên lịch",  cls: "bg-gray-800/60 text-gray-300 border-gray-700/50" },
  RegistrationOpen: { label: "Mở ĐK ngựa",   cls: "bg-blue-900/30 text-blue-300 border-blue-800/50" },
  Ongoing:          { label: "Đang diễn ra", cls: "bg-green-900/30 text-green-300 border-green-800/50" },
  Finished:         { label: "Kết thúc",     cls: "bg-yellow-900/30 text-yellow-300 border-yellow-800/50" },
  Cancelled:        { label: "Đã hủy",       cls: "bg-red-900/30 text-red-300 border-red-800/50" },
};
...
const STATUS_TRANSITIONS = {
  Draft:    [{ label: "Mở đăng ký",    next: "Open",     cls: "bg-blue-700/80 hover:bg-blue-600 text-white" }],
  Open:     [{ label: "Bắt đầu giải",  next: "Ongoing",  cls: "bg-green-700/80 hover:bg-green-600 text-white" }],
  Ongoing:  [{ label: "Kết thúc giải", next: "Finished", cls: "bg-yellow-600/80 hover:bg-yellow-500 text-[#0A0E1A]" }],
  Finished: [],
  Cancelled: [],
};
  ...
  return (
    <AdminLayout title="Chi tiết giải đấu">
      <div className="p-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-start gap-3 mb-6 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-xl font-bold text-white truncate">{tournament.tournamentName}</h1>
          ...
          <div className="flex gap-2 shrink-0 flex-wrap">
            ...
            {canCancel && (
              <button
                ...
                disabled={isChangingStatus}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-red-900/50 hover:bg-red-800 text-red-300 transition-all"
              >
                Hủy giải
              </button>
            )}
        ...
        {/* ── Tabs ── */}
        <div className="flex border-b border-gray-800/60 mb-6 gap-1">
          {TABS.map((tab) => (
            <button
              key={tab.key}
        ...
        {activeTab === "info" && (
          <form onSubmit={handleSaveInfo} className="max-w-xl space-y-4">
            <div className="space-y-1.5">
              <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Tên giải đấu</Label>
              <Input
            ...
            <div className="space-y-1.5">
              <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Địa điểm</Label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Ngày bắt đầu</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Ngày kết thúc</Label>
                <Input
            ...
            <div className="space-y-1.5">
              <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Tổng giải thưởng (VNĐ)</Label>
              <Input
            ...
            <div className="space-y-1.5">
              <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Mô tả</Label>
              <textarea
          ...
          <div>
            ...
            {rounds.length === 0 ? (
              <div className="text-center py-16 text-gray-500">
                <Flag size={40} className="mx-auto mb-3" />
                <p>Chưa có vòng đấu nào. Thêm vòng đấu đầu tiên!</p>
              </div>
            ) : (
              <div className="space-y-3">
                {rounds.map((r) => (
                  <div
                    key={r.roundId}
                    className="flex items-center gap-4 bg-[#111827]/80 border border-gray-800/60 rounded-xl p-4"
                  >
                    <div className="w-8 h-8 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30 flex items-center justify-center text-[#D4AF37] font-bold text-sm shrink-0">
                      {r.roundOrder}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-medium">{r.roundName}</p>
                  ...
                  <tbody>
                    {races.map((race, idx) => {
                      ...
                      return (
                        <tr
                          key={race.raceId}
                          className={`border-b border-gray-800/40 hover:bg-[#111827]/60 transition-colors ${
                            idx % 2 === 0 ? "bg-[#0A0E1A]/60" : "bg-[#0d1220]/60"
                          }`}
                        >
                          <td className="px-4 py-3 text-sm font-medium text-white">{race.raceName}</td>
                          <td className="px-4 py-3 text-sm text-gray-400">{round?.roundName || "—"}</td>
      ...
      {roundModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111827] border border-gray-800/60 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-gray-800/60">
              ...
              <button onClick={() => setRoundModal(null)} className="text-gray-500 hover:text-white text-2xl leading-none">×</button>
            </div>
            <form onSubmit={handleSaveRound} className="p-5 space-y-3">
              {roundFormErr && (
                <div className="flex items-center gap-2 p-2.5 rounded-lg bg-red-950/50 border border-red-900 text-red-200 text-sm">
                  <AlertCircle size={13} /> {roundFormErr}
                </div>
              )}
              <div className="space-y-1.5">
                <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Tên vòng đấu *</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Thứ tự</Label>
                <Input
                  type="number" min="1"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Từ ngày</Label>
                  <Input
                ...
                <div className="space-y-1.5">
                  <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Đến ngày</Label>
                  <Input
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setRoundModal(null)} className="flex-1 h-9 rounded-lg border border-gray-700 text-gray-400 hover:text-white text-sm transition-colors">Hủy</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111827] border border-gray-800/60 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-gray-800/60">
              ...
              <button onClick={() => setRaceModal(null)} className="text-gray-500 hover:text-white text-2xl leading-none">×</button>
            </div>
            <form onSubmit={handleSaveRace} className="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
              ...
              <div className="space-y-1.5">
                <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Tên cuộc đua *</Label>
                <Input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Vòng đấu</Label>
                  <select
                    value={raceModal.data.roundId}
                    onChange={(e) => setRaceModal((p) => ({ ...p, data: { ...p.data, roundId: e.target.value } }))}
                    className="w-full h-10 rounded-md bg-[#0A0E1A]/80 border border-gray-700 text-white text-sm px-3 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]"
                  >
                ...
                <div className="space-y-1.5">
                  <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Ngày thi đấu *</Label>
                  <Input
                    type="datetime-local"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Loại đường đua</Label>
                  <select
                ...
                <div className="space-y-1.5">
                  <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Chiều dài (m)</Label>
                  <Input
                    type="number" min="0"
              ...
              <div className="space-y-1.5">
                <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Số ngựa tối đa</Label>
                <Input
                  type="number" min="1"
              ...
              <div className="grid grid-cols-3 gap-2">
                {[["prizeFirst","Giải 1 (VNĐ)"],["prizeSecond","Giải 2 (VNĐ)"],["prizeThird","Giải 3 (VNĐ)"]].map(([field, lbl]) => (
                  <div key={field} className="space-y-1.5">
                    <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">{lbl}</Label>
                    <Input
                      type="number" min="0"
              ...
              <div className="rounded-lg border border-[#D4AF37]/20 bg-[#D4AF37]/5 p-3 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Mở cổng lúc</Label>
                    <Input
                      type="datetime-local"
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-gray-400 text-xs font-semibold uppercase tracking-widest">Đóng cổng lúc</Label>
                    <Input
                      type="datetime-local"
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setRaceModal(null)} className="flex-1 h-9 rounded-lg border border-gray-700 text-gray-400 hover:text-white text-sm transition-colors">Hủy</button>
      ...
      {refModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111827] border border-gray-800/60 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-gray-800/60">
              <div>
                <h2 className="text-base font-bold text-white">Phân công trọng tài</h2>
                <p className="text-gray-400 text-xs mt-0.5">{refModal.raceName}</p>
              </div>
              <button onClick={() => setRefModal(null)} className="text-gray-500 hover:text-white text-2xl leading-none">×</button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {refModal.assigned.length === 0 ? (
                  <p className="text-gray-600 text-sm">Chưa có trọng tài nào.</p>
                ) : (
                  refModal.assigned.map((ref) => (
                    <div key={ref.refereeId} className="flex items-center justify-between bg-[#0A0E1A]/80 border border-gray-800/60 rounded-lg px-3 py-2">
                      <div>
                        <p className="text-white text-sm font-medium">{ref.fullName || ref.refereeName}</p>
                        <p className="text-gray-500 text-xs">{ref.role}</p>
                      </div>
                      <button onClick={() => handleRemoveRef(ref.refereeId)} className="text-gray-600 hover:text-red-400 transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ))
              ...
              <div className="border-t border-gray-800/60 pt-4">
                <p className="text-gray-400 text-xs font-semibold uppercase tracking-widest mb-3">Thêm trọng tài</p>
                {refErr && (
                  <div className="mb-3 flex items-center gap-2 p-2.5 rounded-lg bg-red-950/50 border border-red-900 text-red-200 text-sm">
                ...
                <form onSubmit={handleAssignRef} className="flex gap-2">
                  <select
                    ...
                    className="flex-1 h-9 rounded-md bg-[#0A0E1A]/80 border border-gray-700 text-white text-sm px-3 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]"
                  >
                  ...
                  <select
                    ...
                    className="w-28 h-9 rounded-md bg-[#0A0E1A]/80 border border-gray-700 text-white text-sm px-2 focus:outline-none focus:ring-1 focus:ring-[#D4AF37]"
                  >
